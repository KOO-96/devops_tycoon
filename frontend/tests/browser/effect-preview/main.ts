 ...
 * isolated effect-preview scenarios against the REAL AssetManager / ProductionImageAssetLoader
 * and the REAL served production effect PNG. Results on window.__effectPreviewResults.
 */
import { Application } from 'pixi.js';
import { runEffectPreviewScenarios, type ScenarioResult } from './scenarios';

interface WebglInfo { webgl2: boolean; renderer?: string }
interface ResultsPayload { webgl: WebglInfo; pixiInit: boolean; scenarios: ScenarioResult[]; errors: string[]; allOk: boolean }
...
async function main(): Promise<void> {
  ...
  const scenarios = await runEffectPreviewScenarios();
  ...
  (window as unknown as { __effectPreviewResults?: ResultsPayload }).__effectPreviewResults = payload;
